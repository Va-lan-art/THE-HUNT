...
@export var es_boton_galeria: bool = false    
@export var es_boton_video: bool = false       # Nueva propiedad para videos
@export var video_path: String = ""            # Ruta del video
...
func _on_button_pressed():
	...
	if es_boton_salida:
		...
		if "anterior" in name.to_lower():  
			emit_signal("galeria_anterior_presionado")
		else:
			emit_signal("galeria_siguiente_presionado")
	elif es_boton_video:  # Nuevo caso para botones de video
		reproducir_video()
	elif nombre_escena != "":
		cambiar_escena()
...
### --- NUEVA FUNCIÓN PARA REPRODUCIR VIDEO --- ###
func reproducir_video():
	if video_path.is_empty():  # Cambiado de empty() a is_empty()
		push_error("No se ha especificado ruta de video")
		return
	
	var video_player_scene = load("res://VideoPlayer.tscn").instantiate()
	video_player_scene.video_path = video_path
	get_tree().root.add_child(video_player_scene)
